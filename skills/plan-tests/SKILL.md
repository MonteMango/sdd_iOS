...
4. **Core mapping (the contract of this skill) — user chooses the level per AC.** Build the AC→test table: **every acceptance criterion in §5 maps to ≥1 test.** For each AC, **propose a default level** from a heuristic (pure logic/rule/validator → unit; behaviour against a real dependency the module owns → integration; a full user-story flow → e2e; a cross-participant API/event shape → contract; and — when a UI surface is declared — a UI piece → component, a user-facing flow → e2e-through-UI). **iOS testing-consultant wiring — before the AskUserQuestion.** Run [`../_shared/consultant-trigger.md`](../_shared/consultant-trigger.md)'s detection against **this one AC's own text** (the `plan-tests` row of its per-stage table) — never the whole §5. On a detected test-strategy signal, consult [`../../agents/swift-testing-consultant.md`](../../agents/swift-testing-consultant.md) by name (`subagent_type: "swift-testing-consultant"`, fallback `general-purpose`) scoped to that AC, fold the returned brief at test-matrix altitude via [`../_shared/consultant-fold.md`](../_shared/consultant-fold.md) — an admitted item (e.g. "this actor-isolated behavior warrants its own dedicated case") shapes the proposed default level before it's shown to the user; a code-altitude item (naming a concrete test tool/framework) is denied entry, left for `implement` to carry (AC-10). No signal on this AC → no consult, no bundle load, no cost (AC-09). **Fallback marker** — bundle-unavailable or a degenerate brief → write it as an HTML comment adjacent to that AC's row in `test-plan.md` (or the inline `## Test plan` section for XS/S), reusing `consultant-fold.md`'s wording, mirrored in the stage handoff; never blocks. Then **confirm the level(s) with the user** via one `AskUserQuestion` (multiSelect — an AC may fan out to several levels, e.g. unit for the rule + e2e for the flow), phrased per [`../_shared/ask-style.md`](../_shared/ask-style.md). The user's choice is authoritative and is recorded in the table's Level column; `implement` reads it to write the test at the right level (it does not re-decide). A criterion with zero tests is the cardinal anti-pattern. Name each test descriptively from the criterion's intent (e.g. `over-quota request is rejected`), not from any framework convention.
