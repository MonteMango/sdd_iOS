...
4.5. **iOS concurrency-consultant — fresh spawn on async behavior.** On a flow carrying async behavior (a suspend point, an actor hop, or a fan-out shape — the same classification step 4 just performed; per [`../_shared/consultant-trigger.md`](../_shared/consultant-trigger.md)'s "the flow being drafted" detection row), spawn a **fresh** [`agents/concurrency-consultant.md`](../../agents/concurrency-consultant.md) call by name (`subagent_type: "concurrency-consultant"`, fallback `general-purpose`) scoped to that one flow's own text — **never reusing `design`'s earlier brief for the same feature**, even if step 3.5 there already consulted it once (spec §3 non-goal 4, accepted duplicate spend, AC-08). Fold the returned brief at **flow-level-detail** altitude via [`../_shared/consultant-fold.md`](../_shared/consultant-fold.md): an admitted flow-specific detail (a suspend point's shape, an actor-hop's ordering, a fan-out's structure) informs step 5's draft of *this* flow; a code-level item (e.g. a concrete retry-loop implementation) is denied entry, left for `implement`/`review` to carry (AC-10). A sync flow (no async signal detected) → no consult, no bundle load, no cost (AC-09). **Fallback marker** — bundle-unavailable or a degenerate brief → write it exactly as `design` already does, an HTML comment in `sad.md` §6 next to this flow (reusing `consultant-fold.md`'s wording), mirrored in the handoff; never blocks step 5.
