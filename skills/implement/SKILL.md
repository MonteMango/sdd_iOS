...
6. **Generate the run-plan.** Sequential → an ordered task list. Team → a shared TaskList with the full task text in each body. Workflow → a generated `Workflow` script (DAG → Kahn phases → fan-out pipeline). **Team and workflow modes precompute each signalled task's own iOS-consultant brief here, before dispatch** — team bakes it into that task's TaskList body, workflow embeds it as that task's own `consultant_brief` field read by the generated `redPrompt(t)` — since neither a dispatched teammate nor a `Workflow` script stage can self-consult (ADR-0001); single-agent mode instead consults inline at its own RED step (step 8 / [`./references/tdd-loop.md`](./references/tdd-loop.md)). → [`./references/team-exec.md`](./references/team-exec.md) / [`./references/workflow-exec.md`](./references/workflow-exec.md).
...
10. **Summary + hand off.** Report covered AC, commits made (with `SDD-Task` trailers), any task dropped/blocked, and the per-task gate results. **Write the pipeline-log section** — create or replace `docs/features/<slug>/pipeline-log.md`'s `### Implement` section per [`../_shared/pipeline-log.md`](../_shared/pipeline-log.md), branching per the mode that **actually ran** this time (ADR-0002): sequential/agent-team sum every `Agent`-tool dispatch's `<usage>` block for the agent count/tokens/duration; dynamic Workflow reads `budget.spent()` for tokens plus the workflow's own agent-call count, marking duration unavailable if the workflow doesn't surface per-call duration. Whichever mode ran, mark any dispatch whose usage didn't return as unavailable per the AC-04 rule (never a false zero), and sum cumulatively if `### Implement` already has a section from a prior run on this feature. This write lands in the same commit as the last task's commit (or its own commit if `auto_commit: off`). Then **emit the stage-handoff block** per [`../_shared/handoff.md`](../_shared/handoff.md) — *What I did* (covered AC, commits with `SDD-Task` trailers, gate results) + *Review* (the committed diff + `tasks/tracker.md` + `pipeline-log.md`) + *Run next* (`/clear`, then `/sdd:review <slug>` — a clean-context pass over the whole diff), then `/sdd:ship <slug>`. In team mode the [`reviewer`](../../agents/reviewer.md) may also run per-task, but the authoritative independent review of the whole change lives in the `review` skill — `implement` does not self-certify.
