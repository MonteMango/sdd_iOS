...
6. **Write the pipeline-log section + rollup.** Create or replace `docs/features/<slug>/pipeline-log.md`'s `### Ship` section per [`../_shared/pipeline-log.md`](../_shared/pipeline-log.md) (agent count/tokens/duration for this run, cumulative on a re-run), then read every backbone-stage (+ `fix`) section currently present and write (or overwrite) the `### Rollup` section: total agent count + sub-agent tokens (summing only available figures, labeled sub-agent-only) + agent-time duration (labeled not-wall-clock), listing any section excluded for unavailable tokens (AC-06b) and any backbone stage with no section (AC-06c). Both writes land in the same commit as the changelog/PR prep.
7. **Summary (terminal handoff).** **Emit the stage-handoff block** per [`../_shared/handoff.md`](../_shared/handoff.md) (terminal variant) — *What I did* (verification result: verified-working / what was deferred and why; the roadmap update) + *Review* (the changelog path + the PR + `pipeline-log.md`) + *Run next* = **Done**: the PR command (or URL if the user ran it) — merging to main is your call; there is no `/sdd` successor.
