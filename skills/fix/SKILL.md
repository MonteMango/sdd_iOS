...
6. **Commit + handoff.** **Write the pipeline-log section** — create or replace `docs/features/<slug>/pipeline-log.md`'s `### Fix` section per [`../_shared/pipeline-log.md`](../_shared/pipeline-log.md) (agent count/tokens/duration for this run, cumulative on a re-run), in the same commit as the fix diff. **If the log already has a `### Rollup` section** (post-ship fix — AC-07), also recompute and overwrite the rollup so it includes this fix's own section, per the rollup rule in [`../_shared/pipeline-log.md`](../_shared/pipeline-log.md). **If no `### Rollup` section exists yet** (pre-ship fix — AC-05b), never create one — that stays `ship`'s job. Propose commit `fix: <slug> <short summary>` with trailers `SDD-Fix: <date>-<short-slug>` and `SDD-AC: <id>` (when an AC was traced). Then **emit the stage-handoff block** per [`../_shared/handoff.md`](../_shared/handoff.md) (utility variant — `/clear` optional): *What I did* + *Review* (the diff, `_fixes/<date>-<short-slug>.md`, the spec patch if any, `pipeline-log.md`) + *Run next*: resume what you were doing; **when the fix touched >5 files or crossed a module boundary, recommend `/sdd:review <slug>`** — a recommendation, not a gate.
