...
1.5. **iOS consultant diff-visible signal (ADR-0005, half of the AND-gate).** Scope the step-1 diff to its **added lines in `.swift` files only** (no context/deleted-line noise — cheap, per ADR-0005). For each of the 3 classes in [`../_shared/consultant-trigger.md`](../_shared/consultant-trigger.md), run its keyword set against those added lines **plus** a second model-inference pass reading the diff's actual code (catches semantic signal a keyword miss — e.g. a `Task {}` block with no literal "async"). A class's **diff-visible** signal affirms when either layer affirms for that class; this produces one diff-visible boolean per class. This step produces the diff-visible signal only — [`./references/review-dimensions.md`](./references/review-dimensions.md) combines it with the spec-visible signal (spec.md prose, the same mechanism `design` already runs) as an AND-gate before step 2 dispatches anything.
2. **Dispatch the independent reviewer.** Before dispatch, resolve the **iOS consultant AND-gate + pre-consult** per [`./references/review-dimensions.md`](./references/review-dimensions.md) (combines step 1.5's diff-visible signal with the spec-visible one; pastes any landed brief into this dispatch, or writes a visible marker for a missing/degenerate consultant — never blocking; a structural-altitude item is denied entry as a finding, AC-10b). Run the [`reviewer`](../../agents/reviewer.md) agent — `subagent_type: "sdd:reviewer"` (read-only, **clean context** — it re-reads spec/contracts itself, no paraphrase; **model per `judgment_model`; effort `xhigh` on L/XL** via `CLAUDE_CODE_EFFORT_LEVEL`, per [`../_shared/agent-roster.md`](../_shared/agent-roster.md)) — over the diff along the dimensions in [`./references/review-dimensions.md`](./references/review-dimensions.md): **stage 1** — every claimed AC genuinely satisfied **and the whole §4 user-story set + §5 AC set traced end-to-end (spec → sequences §6 → data-model → api → tasks → implement): every §4 user story has ≥1 AC and a §6 flow, and every §5 AC reaches code+test — spanning every surface declared in `sad.md` `target_surfaces` (a UI AC traces to a component / e2e-through-UI test, not only a backend one) — flagging any user story or AC that dropped out anywhere in the chain, not only the ACs the diff claims via its `SDD-AC` trailers**; **stage 2** — conventions, error/edge handling, security, boundary violations, test adequacy — **including any landed iOS-consultant findings, which carry the same blocking weight as any other finding (AC-07)**. For a large diff, fan out one reviewer per dimension and merge.
...
5. **Write the review record.** `docs/features/<slug>/_review/review-<date>.md`: scope (diff stat), findings with verdicts, the gate result (`PASS` / `CHANGES REQUESTED`), and — if step 2's pre-consult hit a missing/degenerate consultant for a gated-in class — the visible marker naming it (AC-03), never silently dropped.
6. **Verdict + next.** **Write the pipeline-log section** — create or replace `docs/features/<slug>/pipeline-log.md`'s `### Review` section per [`../_shared/pipeline-log.md`](../_shared/pipeline-log.md) (agent count/tokens/duration for the [`reviewer`](../../agents/reviewer.md) dispatch(es) this run, cumulative on a re-run — a `CHANGES REQUESTED` loop-back that re-invokes `review` accumulates into this same section), in the same commit as `_review/review-<date>.md`. Then **emit the stage-handoff block** per [`../_shared/handoff.md`](../_shared/handoff.md) — *What I did* + *Review* (`_review/review-<date>.md`, `pipeline-log.md`) + *Run next*: `PASS` → (`/clear`, then `/sdd:ship <slug>`); `CHANGES REQUESTED` → `/sdd:implement <slug>` for the fixes (**no `/clear`** — stay in context to iterate), then re-review the changed surface.
