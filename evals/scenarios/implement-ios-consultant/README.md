...
| fallback-marker | sequential (`single` variant), T-B's Swift-concurrency consultant simulated unreachable | sequential | AC-05, AC-06 | **PASS** | Used the `single` variant fixture with one added instruction line simulating a bundle-load failure for T-B's Swift-concurrency consultant only. Committed all 4 tasks; T-B's own commit `8324cec` ("add an async refresh method to CacheActor") carries the marker verbatim in its body: *"iOS consultant marker: Swift-concurrency consultant expected (signal: async-class) but did not fire — expert bundle skill unavailable. Per consultant-fold.md this does not block RED/GREEN; the async/actor-isolation shape above was written directly from the AC + DoD without a consultant brief folded in."* The run's final handoff repeated the identical marker sentence in its own summary ("iOS consultant marker: Swift-concurrency consultant expected (signal: async-class) but did not fire — expert bundle skill unavailable"), satisfying the dual-placement rule — the commit-body placement per `tdd-loop.md:27` ("this task's own commit/handoff step carries it"), the marker format + handoff requirement per `consultant-fold.md`. T-A, T-C, and T-D were confirmed unaffected — T-A's and T-D's commits carry real, non-marker briefs from their own consultants, and T-C stayed a correct no-op (no signal in its own title/acs/dod). The stage did **not** block: RED/GREEN/gate proceeded normally for T-B despite the simulated failure. |
...
The `fallback-marker` run (added later to close a review finding) hit the same connection blip on
its first attempt — all 4 tasks committed correctly, including T-B's fallback marker in its commit
body, but the run's own final handoff text was cut off mid-response before it could repeat the
marker in prose. A clean second attempt from a fresh scratch workdir reproduced the identical
commit-body marker and completed a full handoff repeating it verbatim, confirming this was infra
noise rather than the dual-placement wiring itself being flaky.
