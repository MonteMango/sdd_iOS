...
- **Review correction (2026-07-20):** `pure-logic-spec.md` literally contains the word "UI" four times, always inside a negation ("no UI ... dependency", "not adding a UI"). The keyword rule as originally written (raw substring match) would have affirmatively fired on this text, contradicting the AC-06 PASS recorded above. `consultant-trigger.md` was tightened post-review to whole-word matching + negation-exclusion, so a negated "UI" mention no longer counts as a keyword-layer hit; the model-inference layer independently reads the fixture as pure-logic. The recorded PASS above holds under the corrected rule — it did not hold, unexamined, under the original one.
